...
class AntoshkaUI:
    ...
    def _load_images(self) -> None:
        ...
        if "app_icon_default_256.png" in self.images:
            self.images["app_icon_64"] = self.images["app_icon_default_256.png"].subsample(4, 4)
    ...
    def _draw_gradient(self, event) -> None:
        ...
        for i in range(steps):
            ...
            self.bg.create_rectangle(0, y1, width, y2, fill=color, outline="", tags="grad")
    ...
    def _build_topbar(self) -> None:
        ...
        ttk.Label(top, text="Антошка", font=self.font_title).grid(row=0, column=1, sticky=W, padx=12)
        ttk.Label(
            top,
        ...
        ttk.Button(top, image=gear, command=self._open_settings, style="Icon.TButton").grid(
    ...
    def _build_input(self) -> None:
        ...
        ttk.Button(panel, text="Отправить", command=self._send_text).grid(row=0, column=1, padx=8)
    ...
    def _build_actions(self) -> None:
        ...
        self.listen_button = ttk.Button(row, text="Слушать", image=mic, compound="left", command=self._listen)
        self.listen_button.grid(row=0, column=0, padx=4)

        ttk.Button(row, text="Стоп", image=stop, compound="left", command=self._stop_listen).grid(
            row=0, column=1, padx=4
        )
        ttk.Button(row, text="История", image=hist, compound="left", command=self._open_history).grid(
            row=0, column=2, padx=4
        )
        ttk.Button(row, text="Громкость", image=vol, compound="left", command=self._open_volume).grid(
            row=0, column=3, padx=4
        )
        ttk.Button(row, text="ИИ режим", image=spark, compound="left", command=self._toggle_llm).grid(
    ...
    def _on_listen_result(self, token: int, text: str | None) -> None:
        ...
        if text is None:
            self._set_status("остановлено")
            return
        self.recognized.configure(text=f"Распознанно: «{text}»" if text else "Распознанно: —")
    ...
    def _open_settings(self) -> None:
        ...
        tts_enabled = BooleanVar(value=bool(self.settings.get("tts", {}).get("enabled", True)))
        stt_mode = StringVar(value=str(self.settings.get("stt", {}).get("mode", "text")))
        wake_word = BooleanVar(value=bool(self.settings.get("ui", {}).get("wake_word", False)))

        ttk.Label(win, text="STT режим").grid(row=0, column=0, sticky=W, padx=10, pady=10)
        ttk.Combobox(win, textvariable=stt_mode, values=["text", "vosk"], state="readonly").grid(
